...
if ($Unattended) {
  ...
  Write-Host "  - The 'pj' user is preseeded with the claude_dev SSH key authorised."
  Write-Host ""
  Write-Host "Next (from claude_dev once the VM reboots out of the installer):" -ForegroundColor Cyan
  Write-Host "  ssh pj@<vm-dhcp-ip>     # key auth, no password"
  Write-Host "  rsync -az /home/pj/projects/meridian/ pj@<vm-dhcp-ip>:/home/pj/meridian/"
  Write-Host "  ssh pj@<vm-dhcp-ip>"
  Write-Host "    sudo mv ~/meridian /opt/meridian; sudo chown -R root:root /opt/meridian"
  Write-Host "    cd /opt/meridian"
  Write-Host "    sudo ./install.sh --unattended --config answers.test.env"
  Write-Host ""
  Write-Host "(answers.test.env should set STATIC_IP to a different host than the production .240)"
} else {
  ...
  Write-Host "  - User account: pj  (or anything; needs sudo via install later)"
  ...
  Write-Host ""
  Write-Host "Next (from claude_dev once Debian is up):" -ForegroundColor Cyan
  Write-Host "  rsync -az /home/pj/projects/meridian/ pj@<vm-dhcp-ip>:/home/pj/meridian/"
  Write-Host "  ssh pj@<vm-dhcp-ip>"
  Write-Host "    sudo mv ~/meridian /opt/meridian; sudo chown -R root:root /opt/meridian"
  Write-Host "    cd /opt/meridian"
  Write-Host "    sudo ./install.sh --unattended --config answers.local.env"
  Write-Host ""
  Write-Host "The installer will assign static 192.168.50.240/24 -- active after the next reboot."
