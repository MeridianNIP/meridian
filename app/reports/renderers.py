...
def render_csv(report_type: str, summary: dict, headers: list[str], rows: list[list]) -> tuple[bytes, str]:
    buf = io.StringIO()
    w = csv.writer(buf, quoting=csv.QUOTE_MINIMAL)
    w.writerow(["# report", report_type])
...
def render_html(
    report_type: str, summary: dict, headers: list[str], rows: list[list], *, title: str | None = None
) -> tuple[bytes, str]:
    title = title or report_type.replace("_", " ").title()
    summary_html = "\n".join(
        f"  <dt>{_html_escape(k)}</dt><dd>{_html_escape(v)}</dd>" for k, v in summary.items()
    )
    head_html = "".join(f"<th>{_html_escape(h)}</th>" for h in headers)
    body_html = "\n".join("<tr>" + "".join(f"<td>{_html_escape(c)}</td>" for c in r) + "</tr>" for r in rows)
    doc = f"""<!doctype html>
...
def render(
    report_type: str, summary: dict, headers: list[str], rows: list[list], fmt: str
) -> tuple[bytes, str]:
    if fmt == "csv":
