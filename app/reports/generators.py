...
def monitor_uptime(db: OrmSession, filters: dict) -> tuple[list[str], list[list], dict]:
    ...
    headers = ["Monitor", "Kind", "Target", "Enabled", "Last status",
               "Samples", "OK", "Fail", "Uptime", "Avg value"]
    out_rows = []
...
def cert_expiry(db: OrmSession, filters: dict) -> tuple[list[str], list[list], dict]:
    ...
    headers = ["Subject CN", "Issuer", "Serial", "Valid from",
               "Expires", "Days left", "Kind", "Source"]
    out_rows = []
    ...
    for r in rows:
        ...
        if days_left is None or days_left < 0:
            ...
            healthy += 1
        out_rows.append([
            r.subject_cn, r.issuer_cn or "—", r.serial_hex or "—",
...
def audit_activity(db: OrmSession, filters: dict) -> tuple[list[str], list[list], dict]:
    ...
    out_rows = [[r.actor or "—", r.action, r.n,
                 r.last_ts.isoformat() if r.last_ts else "—"] for r in rows]
    summary = {
        "Window": f"{hours} h",
...
REPORT_REGISTRY: dict[str, dict[str, Any]] = {
    "monitor_uptime": {
        ...
        "filter_schema": [
            {"key": "window_hours", "label": "Window (hours)", "type": "int",
             "min": 1, "max": 8760, "default": 24},
        ],
    },
    "cert_expiry": {
        ...
        "filter_schema": [
            {"key": "horizon_days", "label": "Horizon (days)", "type": "int",
