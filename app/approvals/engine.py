...
def request(
    ...
    *,
    ...
    audit(db, user_id=requester.id, action="approval.requested",
          target_type="approval", target_key=str(appr.id),
          payload={"for_action": action, "for_target": target_key,
                   "expires_in_h": expires_hours},
          justification=justification)
    return appr
...
def decide(
    ...
    *,
    ...
    if appr.expires_at <= now:
        appr.state = "expired"
        audit(db, action="approval.expired",
              target_type="approval", target_key=str(appr.id),
    ...
    audit(db, user_id=approver.id, action=f"approval.{decision}",
          target_type="approval", target_key=str(appr.id),
          payload={"for_action": appr.action, "for_target": appr.target_key,
...
def cancel(db: OrmSession, *, approval_id: uuid.UUID, by: User) -> Approval:
    ...
    audit(db, user_id=by.id, action="approval.cancelled",
          target_type="approval", target_key=str(appr.id),
          payload={"for_action": appr.action})
    return appr
...
def list_pending(db: OrmSession, *, limit: int = 100) -> list[Approval]:
    ...
    return list(db.execute(
        select(Approval).where(
            and_(Approval.state == "pending", Approval.expires_at > now)
        ).order_by(Approval.requested_at.desc()).limit(limit)
    ).scalars())


def list_mine(db: OrmSession, *, user: User, limit: int = 50) -> list[Approval]:
    return list(db.execute(
        select(Approval).where(Approval.requested_by == user.id)
        .order_by(Approval.requested_at.desc()).limit(limit)
    ).scalars())


def get_approved_for(db: OrmSession, *, action: str, target_key: str,
                     requester: User) -> Approval | None:
