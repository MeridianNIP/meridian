...
from app.webhooks.dispatcher import (
    EVENTS, fanout, record_inbound, verify_signature,
)
...
async def list_webhooks(
    ...
    return [
        {
            "id": str(w.id), "direction": w.direction,
            "name": w.name, "description": w.description,
            "url": w.url, "events": list(w.events or []),
            "enabled": w.enabled,
            "has_secret": w.secret_id is not None,
            "created_at": w.created_at.isoformat() if w.created_at else None,
            "updated_at": w.updated_at.isoformat() if w.updated_at else None,
            "inbound_url": (f"/api/v1/webhooks/inbound/{w.id}"
                            if w.direction == "inbound" else None),
        }
        for w in rows
    ]
...
async def create_webhook(
    ...
    secret_id = _store_secret(
        db, name=f"webhook:{body.name}", plaintext=raw_secret,
        category="token", owner_scope="system", created_by=user.id,
    )

    now = datetime.now(timezone.utc)
    wh = Webhook(
        direction=body.direction, name=body.name, description=body.description,
        url=body.url, events=list(body.events), enabled=body.enabled,
        secret_id=secret_id, created_by=user.id,
    ...
    audit(db, user_id=user.id, action="admin.webhook.create",
          target_type="webhook", target_key=str(wh.id),
          payload={"direction": wh.direction, "name": wh.name,
    ...
    return {
        ...
        "inbound_url": (f"/api/v1/webhooks/inbound/{wh.id}"
                        if wh.direction == "inbound" else None),
    }
...
async def update_webhook(
    ...
    if body.name is not None:
        wh.name = body.name; changed["name"] = body.name
    if body.description is not None:
        wh.description = body.description
    if body.url is not None:
        wh.url = body.url; changed["url"] = body.url
    if body.events is not None:
        wh.events = list(body.events); changed["events"] = list(body.events)
    if body.enabled is not None:
        wh.enabled = body.enabled; changed["enabled"] = body.enabled
    ...
    if body.rotate_secret:
        ...
        wh.secret_id = _store_secret(
            db, name=f"webhook:{wh.name}", plaintext=new_secret_plain,
            category="token", owner_scope="system", created_by=user.id,
        )
        changed["secret"] = "rotated"

    db.commit()
    audit(db, user_id=user.id, action="admin.webhook.update",
          target_type="webhook", target_key=str(wh.id),
          payload=changed,
          ip=client_ip(request),
          user_agent=request.headers.get("user-agent"))
...
async def delete_webhook(
    ...
    db.delete(wh)
    db.commit()
    audit(db, user_id=user.id, action="admin.webhook.delete",
          target_type="webhook", target_key=str(webhook_id),
          payload={"name": name},
          ip=client_ip(request),
          user_agent=request.headers.get("user-agent"))
...
async def test_send(
    ...
    if wh.direction != "outbound":
        raise HTTPException(status.HTTP_400_BAD_REQUEST,
                            "test send only applies to outbound webhooks")
    if not wh.url:
    ...
    import httpx
    envelope = {
        "event": body.event, "subject": body.subject,
        "body": "(test)", "payload": body.payload,
    ...
    now = datetime.now(timezone.utc)
    d = WebhookDelivery(
        webhook_id=wh.id, direction="outbound", event=body.event,
        payload=envelope, status="deferred", ts=now, attempt=1,
    )
    try:
    ...
    db.add(d)
    db.commit()
    audit(db, user_id=user.id, action="admin.webhook.test",
          target_type="webhook", target_key=str(wh.id),
          payload={"status": d.status, "http_status": d.http_status,
...
async def list_deliveries(
    ...
    return [
        {
            "id": d.id, "direction": d.direction, "event": d.event,
            "status": d.status, "http_status": d.http_status,
            "response": d.response, "ts": d.ts.isoformat(),
            "attempt": d.attempt,
        }
        for d in rows
    ]
    ...
    try:
        ...
        record_inbound(db, webhook_id=wh.id,
                       ...
                       payload={"error": "invalid json",
                                "content_length": len(body_bytes)},
                       http_status=400, status="failed")
        db.commit()
        raise HTTPException(status.HTTP_400_BAD_REQUEST, "body is not valid JSON")

    event = (payload.get("event") if isinstance(payload, dict) else None) \
            or request.headers.get("X-Meridian-Event", "inbound")
    d = record_inbound(db, webhook_id=wh.id, event=event, payload=payload,
                       http_status=202, status="ok")
    db.commit()
