...
def fanout(
    ...
    *,
    ...
    for wh_id in subs:
        ...
        d = WebhookDelivery(
            webhook_id=wh.id, direction="outbound", event=event,
            payload=envelope, status="deferred",
            ts=now, attempt=1,
        )
        try:
...
def record_inbound(
    ...
    *,
    ...
    d = WebhookDelivery(
        webhook_id=webhook_id, direction="inbound", event=event,
        payload=payload, status=status, http_status=http_status,
        ts=datetime.now(timezone.utc), attempt=1,
    )
    db.add(d)
    return d
