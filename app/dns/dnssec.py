...
_DS_ALG_NAMES = {
    1: "RSAMD5", 5: "RSASHA1", 7: "NSEC3RSASHA1",
...
async def _has_records(zone: str, rtype: str) -> tuple[bool, list[str]]:
    ...
    r = await run_dig(DigRequest(
        target=target, record_type=rtype,
...
async def walk_chain(domain: str) -> DnssecReport:
    ...
    for zone in zones:
        ...
        if not zone:
            ...
            if dnskey_present:
                chain.append(DnssecStep(
                    zone=".", has_ds=True, has_dnskey=True,
                ...
                chain.append(DnssecStep(
                    zone=".", has_ds=False, has_dnskey=False,
                    algorithms=[], outcome="fail",
        ...
        if not dnskey_present and not ds_present:
            ...
            if parent_step and parent_step.has_ds and zone == zones[-1]:
                ...
                chain.append(DnssecStep(
                    zone=zone, has_ds=False, has_dnskey=False,
                    algorithms=[], outcome="fail",
        ...
        if ds_present and not dnskey_present:
            chain.append(DnssecStep(
                zone=zone, has_ds=True, has_dnskey=False,
        ...
        if dnskey_present and not ds_present and zone != zones[-1]:
            ...
            chain.append(DnssecStep(
                zone=zone, has_ds=False, has_dnskey=True,
                algorithms=_extract_algs(dnskey_lines),
                outcome="warn",
        ...
        chain.append(DnssecStep(
            zone=zone, has_ds=ds_present, has_dnskey=dnskey_present,
