...
def _lint_spf_record(raw: str) -> dict[str, Any]:
    ...
    if not raw.startswith("v=spf1"):
        findings.append({"grade": "fail", "check": "Missing version prefix",
                         "detail": "Record must start with `v=spf1` to be recognised.",
                         "fix": "Prefix with `v=spf1 ` (note the trailing space)."})
        return {"raw_record": raw, "rows": findings, "worst": "fail",
                "dns_lookups": 0, "all_qualifier": None, "includes": []}
    ...
    for tok in tokens:
        ...
        if "=" in mech:
            ...
            if name not in _SPF_MODIFIERS:
                findings.append({"grade": "warn", "check": f"unknown modifier {name}",
        ...
        if name not in _SPF_MECHANISMS:
            findings.append({"grade": "warn", "check": f"unknown mechanism {name}",
    ...
    for f in findings:
        if f["grade"] == "fail" or (f["grade"] == "warn" and worst == "ok"):
            worst = f["grade"]
    return {"raw_record": raw, "rows": findings, "worst": worst,
            "dns_lookups": dns_lookups, "all_qualifier": all_mech,
...
def _lint_dmarc_record(raw: str) -> dict[str, Any]:
    ...
    for tag in ("rua", "ruf"):
        ...
        if not v:
            if tag == "rua":
                findings.append({"grade": "warn", "check": "rua missing",
        ...
        for uri in v.split(","):
            ...
            if not (uri.startswith("mailto:") or uri.startswith("https:")):
                findings.append({"grade": "fail", "check": f"{tag} URI format",
...
async def validate_spf(domain: str) -> dict[str, Any]:
    ...
    if not spf_records:
        return {
            "domain": domain, "found": False, "rows": [],
            "rows_total": 0, "worst": "fail",
    ...
    if len(spf_records) > 1:
        findings.insert(0, {"grade": "fail", "check": "Single record",
    ...
    if dns_lookups > 10:
        ...
        findings.append({"grade": "ok", "check": "DNS-lookup count",
                         "detail": f"{dns_lookups} of 10 lookups used.", "fix": ""})
    ...
    return {
        ...
        "raw_record": raw, "dns_lookups": dns_lookups,
        "includes": lint["includes"], "all_qualifier": lint["all_qualifier"],
        "rows": findings, "worst": worst,
    }
...
async def validate_dkim(domain: str, selector: str) -> dict[str, Any]:
    ...
    if not records:
        return {
            "domain": domain, "selector": selector, "host": host,
            "found": False, "rows": [], "worst": "fail",
    ...
    return {
        "domain": domain, "selector": selector, "host": host,
        "found": True, "raw_record": raw, "tags": lint["tags"],
        "rows": lint["rows"], "worst": lint["worst"],
    }
...
async def validate_dmarc(domain: str) -> dict[str, Any]:
    ...
    if not records:
        return {
            "domain": domain, "host": host, "found": False,
            "rows": [], "worst": "fail",
    ...
    return {
        "domain": domain, "host": host, "found": True,
        "raw_record": raw, "tags": lint["tags"], "policy": lint["policy"],
        "rows": lint["rows"], "worst": lint["worst"],
    }
