...
def _load_active_panel(user_id=None, group_tag: str | None = None) -> tuple[tuple[str, str], ...]:
    ...
    try:
        ...
        with session_scope() as db:
            ...
            stmt = select(Resolver.name, Resolver.ip).where(Resolver.is_propagation_default.is_(True), cond)
            if group_tag:
...
async def check_propagation(
    target: str, record_type: str = "A", *, user_id=None, group_tag: str | None = None
) -> PropagationReport:
    from app.safety.limits import (
        PROPAGATION_CONCURRENCY,
        PROPAGATION_MAX_RESOLVERS,
        PROPAGATION_PER_RESOLVER_TIMEOUT_S,
        bounded_gather,
    )
    ...
    raw_results: list[DigResult] = await bounded_gather(
        tasks,
        max_workers=PROPAGATION_CONCURRENCY,
        return_exceptions=False,
    )

    rows: list[PropRow] = []
    answers: set[str] = set()
    for (provider, ip), r in zip(panel, raw_results, strict=False):
