...
async def check_zone(domain: str) -> ZoneReport:
    ...
    # 1 · NS delegation
    ns_res = await run_dig(DigRequest(target=domain, record_type="NS", flags=("+short", "+noall", "+answer")))
    ns_list = [l.rstrip(".") for l in _extract_all(ns_res.stdout)]
    if not ns_list:
        findings.append(ZoneFinding("fail", "ns_delegation", f"no NS records returned for {domain}"))
        return ZoneReport(domain=domain, ns_records=[], soa_serials={}, findings=findings, worst="fail")
    findings.append(ZoneFinding("info", "ns_delegation", f"{len(ns_list)} NS records", {"ns": ns_list}))
    ...
    if len(unique) == 0:
        findings.append(
            ZoneFinding("warn", "soa_serial", "could not retrieve SOA from any NS", {"serials": serials})
        )
    elif len(unique) > 1:
        findings.append(
            ZoneFinding(
        ...
        findings.append(
            ZoneFinding(
                "info", "soa_serial", f"all NSes report serial {next(iter(unique))}", {"serials": serials}
            )
        )

    # 3 · CNAME/A sanity on the apex — apex CNAME violates RFC 1034.
    cname_res = await run_dig(
        DigRequest(target=domain, record_type="CNAME", flags=("+short", "+noall", "+answer"))
    )
    ...
    # 4 · Apex A/AAAA
    a_res = await run_dig(DigRequest(target=domain, record_type="A", flags=("+short", "+noall", "+answer")))
    aaaa_res = await run_dig(
        DigRequest(target=domain, record_type="AAAA", flags=("+short", "+noall", "+answer"))
    )
    has_a = bool(_extract_first(a_res.stdout))
    has_aaaa = bool(_extract_first(aaaa_res.stdout))
    if not has_a and not has_aaaa:
        findings.append(ZoneFinding("warn", "apex_address", "no A or AAAA record at apex"))
    else:
        findings.append(
            ZoneFinding(
                "info",
    ...
    mx_res = await run_dig(DigRequest(target=domain, record_type="MX", flags=("+short", "+noall", "+answer")))
    if not _extract_first(mx_res.stdout):
        findings.append(ZoneFinding("info", "mx", "no MX records (zone doesn't accept mail)"))
    else:
        findings.append(ZoneFinding("info", "mx", f"{len(_extract_all(mx_res.stdout))} MX records"))

    worst_order = {"info": 0, "warn": 1, "fail": 2}
    worst = max(findings, key=lambda f: worst_order[f.severity]).severity
    return ZoneReport(domain=domain, ns_records=ns_list, soa_serials=serials, findings=findings, worst=worst)
