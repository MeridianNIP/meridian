...
_FIELDS: tuple[tuple[str, tuple[str, ...]], ...] = (
    ...
    (
        "expires",
        ("Registry Expiry Date:", "Expiration Date:", "Expires On:", "Expires:", "Expiration:", "paid-till:"),
    ),
...
async def whois_bulk(targets: list[str], *, timeout_s: float = 10.0, concurrency: int = 4) -> dict[str, Any]:
...
async def typosquat_scan(target: str, *, max_variants: int = 150, timeout_s: float = 3.0) -> dict[str, Any]:
    if not _DOMAIN_RE.match(target):
        raise ValueError(f"not a valid domain: {target!r}")
