...
async def axfr_audit(domain: str) -> AxfrReport:
    ...
    nameservers = [
        l.strip().rstrip(".") for l in ns_res.stdout.splitlines() if l.strip() and not l.startswith(";")
    ]
    ...
    async def _try(ns: str) -> AxfrRow:
        ...
        stdout = r.stdout
        if "Transfer failed" in stdout or "communications error" in stdout or "refused" in stdout.lower():
        ...
        if len(records) >= 2:
            return AxfrRow(
                nameserver=ns, exposed=True, detail=f"EXPOSED · {len(records)} records transferred"
            )
        return AxfrRow(nameserver=ns, exposed=False, detail="no transfer (empty response)")
