...
def _load_secret(db: OrmSession, secret_id) -> str | None:
    ...
    if row is None:
        return None
    try:
        return decrypt_field(bytes(row.nonce) + bytes(row.ciphertext),
                             domain=b"vault").decode()
    ...
    *,
    ...
    try:
        ...
        return DeviceBackupResult(
            ...
            ok=False, changed=False, snapshot_id=None,
            sha256=None, size_bytes=None, error=err,
    ...
    if not changed:
        return DeviceBackupResult(
            device_id=str(device.id), device_name=device.name,
            ok=True, changed=False, snapshot_id=None,
            sha256=sha256, size_bytes=len(raw.encode()),
            error=None, duration_ms=int((time.monotonic() - t0) * 1000),
        )
    ...
    body = (f"Device: {device.name} ({device.kind} @ {device.mgmt_host})\n"
            f"Trigger: {trigger}\n"
    ...
    payload = {
        "device_id": str(device.id), "device_name": device.name,
        "kind": device.kind, "mgmt_host": device.mgmt_host,
        "snapshot_id": str(snap.id), "sha256": sha256,
        "lines_added": added, "lines_removed": removed,
        "trigger": trigger,
    }
    try:
        from app.notifications.dispatcher import dispatch
        dispatch(db, event_kind="device.config_changed",
                 subject=subject, body=body, payload=payload)
    except Exception:  # noqa: BLE001
        pass
    try:
        from app.webhooks.dispatcher import fanout
        fanout(db, event="device.config_changed",
               payload=payload, subject=subject, body=body)
    except Exception:  # noqa: BLE001
        pass

    audit(db, action="device.config.snapshot",
          target_type="network_device", target_key=device.name,
          payload={"snapshot_id": str(snap.id), "sha256": sha256,
                   "lines_added": added, "lines_removed": removed,
                   "trigger": trigger})
    ...
    return DeviceBackupResult(
        device_id=str(device.id), device_name=device.name,
        ok=True, changed=True, snapshot_id=str(snap.id),
        sha256=sha256, size_bytes=len(raw.encode()),
        error=None,
...
def backup_all(
    ...
    *,
    ...
    run = DeviceBackupRun(
        started_at=started, trigger_kind=trigger,
    )
    db.add(run)
    db.flush()
    ...
    audit(db, action="device.backup.run",
          target_type="device_backup_run", target_key=str(run.id),
    ...
    return {
        ...
        "results": [
            {"device_id": r.device_id, "device_name": r.device_name,
             "ok": r.ok, "changed": r.changed, "snapshot_id": r.snapshot_id,
