...
from app.models.device import (
    DeviceBackupRun, DeviceConfigSnapshot, NetworkDevice,
)
...
_ALLOWED_KINDS = (
    # Cisco
    "cisco_ios", "cisco_iosxe", "cisco_iosxr", "cisco_nxos", "cisco_asa",
    "cisco_wlc", "cisco_s300",
    # Other enterprise
    "juniper_junos", "arista_eos", "palo_alto", "fortinet", "huawei",
    "aruba_aoscx", "aruba_os", "hp_procurve", "hp_comware",
    "dell_os10", "dell_force10", "dell_powerconnect",
    "extreme_exos", "brocade_fastiron",
    # ADC
    "f5_tmsh", "citrix_netscaler",
    # Firewalls
    "sonicwall", "pfsense", "opnsense", "sophos",
    # SoHo / open-source
    "mikrotik", "ubiquiti_edge", "ubiquiti_unifi", "vyos",
    # NAS
    "synology", "qnap",
    # Generic
    "generic_ssh",
...
def _serialize(d: NetworkDevice, *, include_tags: bool = True) -> dict:
    return {
        ...
        "kind": d.kind, "mgmt_host": d.mgmt_host, "mgmt_port": d.mgmt_port,
        "username": d.username, "has_secret": d.secret_id is not None,
        "has_enable_secret": d.enable_secret_id is not None,
        "enabled": d.enabled, "auto_backup": d.auto_backup,
...
async def create_device(
    ...
    if body.password:
        secret_id = _store_secret(db, name=f"device:{body.name}",
                                  plaintext=body.password, category="password",
                                  owner_scope="system", created_by=user.id)
    if body.enable_password:
        enable_secret_id = _store_secret(db, name=f"device-enable:{body.name}",
    ...
    d = NetworkDevice(
        ...
        mgmt_host=body.mgmt_host, mgmt_port=body.mgmt_port,
        username=body.username, secret_id=secret_id,
        enable_secret_id=enable_secret_id,
        enabled=body.enabled, auto_backup=body.auto_backup,
        retain_snapshots_count=body.retain_snapshots_count,
        tags=list(body.tags), site=body.site, config=body.config,
        created_by=user.id, created_at=now, updated_at=now,
    )
    db.add(d)
    db.commit()
    db.refresh(d)
    audit(db, user_id=user.id, action="device.create",
          target_type="network_device", target_key=d.name,
          payload={"kind": d.kind, "mgmt_host": d.mgmt_host,
...
async def update_device(
    ...
    changed: dict = {}
    for field in ("description", "mgmt_host", "mgmt_port", "username",
                  "enabled", "auto_backup", "retain_snapshots_count", "site"):
        v = getattr(body, field)
        if v is not None:
            setattr(d, field, v)
            changed[field] = v
    if body.tags is not None:
        d.tags = list(body.tags); changed["tags"] = list(body.tags)
    if body.config is not None:
        d.config = body.config; changed["config_keys"] = sorted(body.config.keys())

    if body.clear_password:
        _delete_secret(db, d.secret_id)
        d.secret_id = None; changed["password"] = "cleared"
    elif body.password:
        _delete_secret(db, d.secret_id)
        d.secret_id = _store_secret(db, name=f"device:{d.name}",
                                    plaintext=body.password, category="password",
                                    owner_scope="system", created_by=user.id)
    ...
    if body.clear_enable_password:
        ...
        d.enable_secret_id = None; changed["enable_password"] = "cleared"
    elif body.enable_password:
        _delete_secret(db, d.enable_secret_id)
        d.enable_secret_id = _store_secret(db, name=f"device-enable:{d.name}",
                                           plaintext=body.enable_password,
                                           category="password", owner_scope="system",
                                           created_by=user.id)
    ...
    db.commit()
    audit(db, user_id=user.id, action="device.update",
          target_type="network_device", target_key=d.name,
...
async def delete_device(
    ...
    db.delete(d)
    db.commit()
    audit(db, user_id=user.id, action="device.delete",
          target_type="network_device", target_key=name,
          ip=client_ip(request),
          user_agent=request.headers.get("user-agent"))
...
async def backup_now(
    ...
    db.commit()
    audit(db, user_id=user.id, action="device.backup.manual",
          target_type="network_device", target_key=d.name,
          payload={"ok": result.ok, "changed": result.changed,
                   "sha256": result.sha256, "error": result.error},
          ip=client_ip(request),
          user_agent=request.headers.get("user-agent"),
          outcome="ok" if result.ok else "error")
    return {
        "ok": result.ok, "changed": result.changed,
        "snapshot_id": result.snapshot_id, "sha256": result.sha256,
        "size_bytes": result.size_bytes, "error": result.error,
...
async def list_snapshots(
    ...
    return [
        {
            "id": str(s.id), "ts": s.ts.isoformat(),
            "trigger_kind": s.trigger_kind, "size_bytes": s.size_bytes,
            "sha256_hex": s.sha256_hex, "line_count": s.line_count,
...
async def get_snapshot(
    ...
    return {
        "id": str(s.id), "device_id": str(s.device_id),
        "ts": s.ts.isoformat(), "trigger_kind": s.trigger_kind,
        "raw_config": s.raw_config, "size_bytes": s.size_bytes,
        "sha256_hex": s.sha256_hex, "line_count": s.line_count,
        "prev_snapshot_id": str(s.prev_snapshot_id) if s.prev_snapshot_id else None,
...
async def diff_snapshots(
    ...
    return {
        "from": {"id": str(sa.id), "ts": sa.ts.isoformat(),
                 "sha256": sa.sha256_hex},
        "to":   {"id": str(sb.id), "ts": sb.ts.isoformat(),
                 "sha256": sb.sha256_hex},
        "diff": diff_text,
...
async def list_runs(
    ...
    rows = db.execute(
        select(DeviceBackupRun).order_by(DeviceBackupRun.started_at.desc()).limit(limit)
    ).scalars().all()
    return [
        {
            "id": str(r.id), "started_at": r.started_at.isoformat(),
