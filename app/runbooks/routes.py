...
async def list_runbooks(
    ...
    if mine_only:
        ...
        stmt = stmt.where(or_(Runbook.owner_id == user.id, Runbook.shared.is_(True)))
...
async def create_runbook(
    ...
    audit(
        ...
        target_type="runbook",
        target_key=str(rb.id),
        payload={"name": rb.name, "step_count": len(rb.steps or []), "shared": rb.shared},
...
async def update_runbook(
    ...
    if rb.owner_id != user.id and user.role not in ("admin", "super_admin"):
        raise HTTPException(status.HTTP_403_FORBIDDEN, "only the owner or an admin may edit this runbook")
    changed: dict = {}
    ...
    if body.shared is not None:
        if (
            body.shared
            and "runbook.share" not in {p.key for p in user.permissions}
            and user.role not in ("admin", "super_admin")
        ):  # type: ignore[attr-defined]
            raise HTTPException(status.HTTP_403_FORBIDDEN, "sharing requires runbook.share permission")
        rb.shared = body.shared
        changed["shared"] = body.shared
    if body.enabled is not None:
        rb.enabled = body.enabled
        changed["enabled"] = body.enabled
    if body.steps is not None:
        rb.steps = [s.model_dump() for s in body.steps]
        changed["step_count"] = len(rb.steps)
    rb.updated_at = datetime.now(UTC)
    db.commit()
    audit(
        db,
        user_id=user.id,
...
async def delete_runbook(
    ...
    if rb is None:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "runbook not found")
    if rb.owner_id != user.id and user.role not in ("admin", "super_admin"):
        raise HTTPException(status.HTTP_403_FORBIDDEN, "only the owner or an admin may delete this runbook")
    name = rb.name
    db.delete(rb)
    db.commit()
    audit(
