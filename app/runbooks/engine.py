...
async def run_runbook(
    *, runbook: Runbook, user: User, db: OrmSession,
) -> dict:
    ...
    for idx, step in enumerate(runbook.steps or []):
        ...
        if tool is None:
            res = {"outcome": "error", "summary": f"unknown tool {tool_key!r}",
                   "detail": {}}
            ...
            try:
                res = await tool.execute(
                    ...
                    user=user, db=db, scope=scope_of_use,
                )
    ...
    status = "ok" if worst in ("ok", "info") else \
             "warn" if worst == "warn" else \
             "fail"
    ...
    """), {
        "id": run_id, "rb": runbook.id, "u": user.id,
        "t0": started, "t1": completed, "s": status,
        "r": __import__("json").dumps(results),
    })

    audit(db, user_id=user.id, action="runbook.run",
          target_type="runbook", target_key=str(runbook.id),
          payload={"run_id": str(run_id),
                   "step_count": len(runbook.steps or []),
                   "executed": len(results),
                   "status": status, "stopped_early": stopped_early})
