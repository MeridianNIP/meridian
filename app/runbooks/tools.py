...
async def _tool_dig(params: dict, *, user: User, db: OrmSession, scope: str | None) -> dict:
    ...
    return {
        ...
        "detail": {"command": res.command, "rows": rows[:20],
...
async def _tool_propagation(params: dict, *, user: User, db: OrmSession, scope: str | None) -> dict:
    ...
    return {
        ...
        "detail": {"unique_answers": list(rep.unique_answers),
                   "ok_count": ok, "total": len(rep.rows),
...
async def _tool_ping(params: dict, *, user: User, db: OrmSession, scope: str | None) -> dict:
    ...
    res = await run_ping(
        PingRequest(target=params["target"],
    ...
    return {
        ...
        "detail": {"rtt_avg": res.stats.rtt_avg, "rtt_min": res.stats.rtt_min,
                   "rtt_max": res.stats.rtt_max, "jitter": res.stats.jitter,
                   "loss_pct": loss},
    }
...
async def _tool_traceroute(params: dict, *, user: User, db: OrmSession, scope: str | None) -> dict:
    ...
    res = await run_traceroute(
        TraceRequest(target=params["target"],
    ...
    return {
        ...
        "summary": f"{len(res.hops)} hops"
                   + (f" · last: {res.hops[-1].host}" if res.hops else ""),
        "detail": {"hops": [{"ttl": h.ttl, "host": h.host, "ip": h.ip,
                             "rtts_ms": list(h.rtts_ms)} for h in res.hops]},
    }
...
async def _tool_http(params: dict, *, user: User, db: OrmSession, scope: str | None) -> dict:
    ...
    return {
        ...
        "detail": {"final_url": res.final_url,
...
async def _tool_port_scan(params: dict, *, user: User, db: OrmSession, scope: str | None) -> dict:
    ...
    res = await scan(params["target"], port_list,
                     timeout_s=float(params.get("timeout_s", 2.0)),
    ...
    return {
        ...
        "detail": {"open_ports": list(res.open_ports),
                   "ports_scanned": res.ports_scanned},
    }
...
async def _tool_dnssec(params: dict, *, user: User, db: OrmSession, scope: str | None) -> dict:
    ...
    return {
        ...
        "detail": {"ad_flag": rep.ad_flag,
                   "chain": [{"zone": s.zone, "outcome": s.outcome,
                              "has_ds": s.has_ds, "has_dnskey": s.has_dnskey,
                              "algorithms": s.algorithms,
                              "message": s.message} for s in rep.chain]},
    }
...
async def _tool_axfr(params: dict, *, user: User, db: OrmSession, scope: str | None) -> dict:
    ...
    return {
        "outcome": sev,
        "summary": ("AXFR exposed on at least one NS" if rep.any_exposed
                    else f"{len(rep.rows)} NS tested, all refused"),
        "detail": {"any_exposed": rep.any_exposed,
                   "rows": [{"ns": r.nameserver, "exposed": r.exposed,
                             "detail": r.detail} for r in rep.rows]},
    }
...
async def _tool_wizard(params: dict, *, user: User, db: OrmSession, scope: str | None) -> dict:
    ...
    try:
        ...
        return {"outcome": "fail", "summary": f"unknown wizard: {e}",
                "detail": {"error": str(e)}}
    return {
        ...
        "detail": {"run_id": res["run_id"], "outcome": res["outcome"],
...
async def _tool_assert_ip(params: dict, *, user: User, db: OrmSession, scope: str | None) -> dict:
    ...
    try:
        ...
        return {"outcome": "fail", "summary": f"expected_ip is not an IP: {expected}",
                "detail": {"expected": expected}}
    res = await run_dig(DigRequest(target=params["target"], record_type="A"))
    rows = [l.strip() for l in res.stdout.splitlines()
...
# ----------------------------------------------------------------------------
register(ToolSpec(
    key="dig", label="DNS dig", category="DNS",
    ...
    params=[
        ...
        ToolParam("record_type", "Record type", "enum", default="A",
...
register(ToolSpec(
    key="propagation", label="Global propagation", category="DNS",
...
register(ToolSpec(
    key="zone_health", label="Zone health check", category="DNS",
...
register(ToolSpec(
    key="dnssec", label="DNSSEC chain walker", category="DNS",
...
register(ToolSpec(
    key="axfr", label="AXFR audit", category="DNS",
...
register(ToolSpec(
    key="ping", label="Ping", category="Network",
...
register(ToolSpec(
    key="traceroute", label="Traceroute", category="Network",
...
register(ToolSpec(
    key="port_scan", label="Port scan", category="Network",
...
register(ToolSpec(
    key="wizard", label="Run wizard", category="Workflow",
    description="Invoke a registered wizard by key (e.g. dns.resolve_fail).",
    required_permission="dns.sandbox",
    params=[
        ToolParam("wizard_key", "Wizard key", "text", required=True,
...
register(ToolSpec(
    key="assert_ip", label="Assert A record", category="Workflow",
...
register(ToolSpec(
    key="sleep", label="Sleep", category="Workflow",
