...
async def login(
    ...
    if not local_ok:
        ...
        ad_result = try_ad_authenticate(db, username, password, ip=ip, user_agent=ua)
        if ad_result is not None:
            ...
            if user is not None:
                ...
                audit(
                    db,
                    action="auth.login.failed",
                    payload={"username": username, "reason": "no_such_user_or_disabled", "src": ip or "-"},
                    ip=ip,
                    user_agent=ua,
                    outcome="denied",
                )
    ...
    if user.mfa_enrolled:
        ...
        if user.mfa_secret_enc is None or not verify_totp(decrypt_totp_secret(user.mfa_secret_enc), mfa_code):
            audit(
                db,
                user_id=user.id,
                action="auth.login.failed",
...
async def mfa_begin(
    ...
    if user.mfa_enrolled:
        raise HTTPException(status.HTTP_400_BAD_REQUEST, "already enrolled — disable first to re-enroll")
    secret = generate_totp_secret()
    # Stash encrypted; mfa_enrolled stays False until confirm() succeeds.
    u = db.get(User, user.id)
    u.mfa_secret_enc = encrypt_totp_secret(secret)
    db.flush()
    uri = provisioning_uri(user.username, secret, issuer=get_settings().portal_name or "Meridian")
...
# ============================================================================
from app.auth import recovery as _rec
...
async def recovery_setup(
    ...
    audit(
        db,
        user_id=user.id,
        action="recovery.questions.setup",
        target_type="user",
...
async def recovery_challenge(
    ...
    # Lookup by username.
    user = db.execute(
        __import__("sqlalchemy").select(_UserModel).where(_UserModel.username == body.username)
    ).scalar_one_or_none()
...
async def recovery_verify(
    ...
    if user is not None and _rec.is_enrolled(db, user.id):
        if _rec.recent_failures(db, user.id) < 5:
            ...
            if _rec.verify_challenge(db, user.id, answers):
                ...
                _rec.record_attempt(db, user.id, outcome="ok", ip=client_ip(request))
                audit(
                    db,
                    user_id=user.id,
                    action="recovery.challenge.ok",
                    target_type="user",
                    target_key=str(user.id),
    ...
    if failed:
        if user is not None:
            _rec.record_attempt(db, user.id, outcome="fail", ip=client_ip(request))
            audit(
                db,
                user_id=user.id,
                action="recovery.challenge.fail",
                target_type="user",
                target_key=str(user.id),
    ...
    return {"ok": True, "reset_token": token, "expires_in_minutes": _rec.RESET_TOKEN_TTL_MIN}
...
async def recovery_reset(
    ...
    if user_id is None:
        raise HTTPException(status.HTTP_400_BAD_REQUEST, "reset token is invalid, used, or expired")
    ...
    audit(
        db,
        user_id=user_id,
        action="recovery.password.reset",
        target_type="user",
