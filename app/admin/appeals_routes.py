...
async def list_appeals(
    ...
    if status_filter not in (*_VALID_STATUSES, "all"):
        raise HTTPException(status.HTTP_400_BAD_REQUEST,
                            f"status_filter must be one of {_VALID_STATUSES} or 'all'")
    ...
    if status_filter == "all":
        ...
        rows = db.execute(text(
            ...
            " ORDER BY submitted_at DESC LIMIT :n"
        ), {"s": status_filter, "n": limit}).mappings().all()
...
def _set_status(db, appeal_id, new_status, note, user, request) -> dict:
    ...
    ), {"s": new_status, "u": user.id, "n": note, "id": appeal_id}).rowcount
    if n == 0:
        raise HTTPException(status.HTTP_404_NOT_FOUND,
                            "appeal not found or already resolved")
    audit(db, user_id=user.id, action=f"admin.lockout_appeal.{new_status}",
