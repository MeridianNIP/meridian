...
async def stats(
    ...
    return {
        "users_enabled": db.execute(
            text("SELECT count(*) FROM users WHERE enabled AND deleted_at IS NULL")
        ).scalar_one(),
        "sessions_active": db.execute(
            text("SELECT count(*) FROM sessions WHERE revoked_at IS NULL AND expires_at > now()")
        ).scalar_one(),
        "monitors_enabled": db.execute(text("SELECT count(*) FROM monitors WHERE enabled")).scalar_one(),
        "audit_24h": db.execute(
            text("SELECT count(*) FROM audit_events WHERE ts > :t"), {"t": day_ago}
        ).scalar_one(),
