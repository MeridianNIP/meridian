...
async def patch_policy(
    ...
    for field, val in data.items():
        ...
        if field == "mfa_requirement" and val not in _MFA_CHOICES:
            raise HTTPException(status.HTTP_400_BAD_REQUEST, f"mfa_requirement must be one of {_MFA_CHOICES}")
        if field == "lockout_unlock_mode" and val not in _UNLOCK_CHOICES:
            raise HTTPException(
                status.HTTP_400_BAD_REQUEST, f"lockout_unlock_mode must be one of {_UNLOCK_CHOICES}"
            )
        setattr(b, field, val)
        changed[field] = val
