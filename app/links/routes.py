...
async def create_link(
    ...
    if body.scope == "global" and not _is_admin(user):
        raise HTTPException(status.HTTP_403_FORBIDDEN, "only admin / super_admin can create global links")
    link = ImportantLink(
        scope=body.scope,
    ...
    audit(
        ...
        user_id=user.id,
        action="link.create",
        target_type="important_link",
        target_key=link.name,
...
async def update_link(
    ...
    if link.scope == "global":
        if not _is_admin(user):
            raise HTTPException(status.HTTP_403_FORBIDDEN, "admin role required to edit global link")
    elif link.user_id != user.id:
        raise HTTPException(status.HTTP_403_FORBIDDEN, "this personal link belongs to a different user")
...
async def delete_link(
    ...
    if link.scope == "global":
        if not _is_admin(user):
            raise HTTPException(status.HTTP_403_FORBIDDEN, "admin role required to delete global link")
    elif link.user_id != user.id:
        raise HTTPException(status.HTTP_403_FORBIDDEN, "this personal link belongs to a different user")
    name = link.name
    db.delete(link)
