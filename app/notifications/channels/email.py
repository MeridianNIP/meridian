...
async def _async_send(*, host: str, port: int, use_tls: bool, from_addr: str,
                      to_addr: str, subject: str, body: str,
                      username: str | None, password: str | None) -> None:
    msg = _build_message(from_addr=from_addr, to_addr=to_addr,
    ...
    *,
    ...
    if channel.secret_id is not None:
        ...
        if row is not None:
            ...
            try:
                password = decrypt_field(bytes(row.nonce) + bytes(row.ciphertext),
    ...
    try:
        asyncio.run(_async_send(
            ...
            subject=subject, body=body,
            username=cfg.get("username"), password=password,
        ))
    except Exception as e:  # noqa: BLE001
        d.status = "failed"
