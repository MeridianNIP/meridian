...
async def network_reachability(ctx: WizardContext) -> list[Suggestion]:
    ...
    if not is_ip:
        ...
        if not rows:
            ctx.add(WizardStep(name="DNS resolves", outcome="fail", message=f"{target} has no A record"))
            return [
                Suggestion(
                    priority="critical",
                    title="DNS is the first wall",
        ...
        ctx.add(
            WizardStep(
                name="DNS resolves", outcome="ok", message=f"{target} → {rows[0]}", detail={"answers": rows}
            )
        )
    else:
        ctx.add(WizardStep(name="DNS resolves", outcome="info", message="target is a literal IP — skipped"))
    ...
    trace = await run_traceroute(TraceRequest(target=target, max_hops=20, timeout_s=3, per_hop_probes=2))
    ctx.add(
        WizardStep(
            name="Traceroute",
            outcome="ok" if trace.hops else "warn",
            message=f"{len(trace.hops)} hops observed"
            + (f" · last hop: {trace.hops[-1].host}" if trace.hops else ""),
        )
    )
    ...
    try:
        ...
        ctx.add(
            WizardStep(
                ...
                message=(
                    f"Open: {open_ports}"
                    if open_ports
                    else "None of 443/80/22 open — host may be firewalled or serving custom ports"
                ),
            )
        )
        ...
        ctx.add(
            WizardStep(
                name="Common TCP ports", outcome="warn", message=f"Port scan rejected by scope policy: {e}"
            )
        )
        open_ports = []
    ...
    if 443 in open_ports or 80 in open_ports:
        ...
        try:
            http = await test_url(f"{scheme}://{target}/", timeout_s=8.0, follow_redirects=True)
            ctx.add(
                WizardStep(
            ...
            ctx.add(WizardStep(name="HTTP request", outcome="warn", message=f"Request failed: {e}"))
    return []
...
async def up_for_everyone(ctx: WizardContext) -> list[Suggestion]:
    ...
    # Local probe
    try:
        local = await test_url(f"https://{target}/", timeout_s=6.0, follow_redirects=True)
        local_ok = 200 <= local.final_status < 500
        ctx.add(
            WizardStep(
                name="Local probe (this Meridian host)",
        ...
        local_ok = False
        ctx.add(WizardStep(name="Local probe (this Meridian host)", outcome="fail", message=f"failed: {e}"))
    ...
    ctx.add(
        WizardStep(
            ...
            message=(
                f"{len(prop.rows)} resolvers → {prop.unique_answers}"
                if external_ok
                else "No public resolver returned an answer"
            ),
        )
    )
    ...
    if local_ok and external_ok:
        return [
            Suggestion(
                priority="info",
                title="Reachable from here and globally",
...
async def ip_reputation(ctx: WizardContext) -> list[Suggestion]:
    ...
    try:
        ...
        if not rows:
            ctx.add(
                WizardStep(
                    name="resolve target", outcome="fail", message=f"Could not resolve {target} to an IP"
                )
            )
            return []
        target = rows[0]
        ctx.add(WizardStep(name="resolved target", outcome="info", message=f"{ctx.target} → {target}"))
    ...
    ctx.add(
        WizardStep(
            ...
            message=(
                f"{who_data.get('org', '?')} · {who_data.get('country', '?')} · {who_data.get('region', '')}"
                if who_data and "error" not in who_data
                else f"lookup failed: {who_data.get('error', 'unknown')}"
            ),
            detail=who_data,
        )
    )
    ...
    ctx.add(
        WizardStep(
            ...
            message=(
                f"{len(ports)} open ports · {len(cves)} known CVEs" if ports or cves else "no public signal"
            ),
            detail={
                "ports": ports[:30],
                "cves": cves[:30],
