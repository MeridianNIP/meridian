...
async def mail_flow_validator(ctx: WizardContext) -> list[Suggestion]:
    ...
    if not mx_rows:
        ctx.add(WizardStep(name="MX lookup", outcome="fail", message=f"No MX for {domain}"))
        return [
            Suggestion(
                priority="critical",
                title="No MX record",
    ...
    ctx.add(WizardStep(name="MX lookup", outcome="ok", message=f"{len(mx_rows)} MX: {mx_rows[:3]}"))
        ...
        ctx.add(
            WizardStep(
                ...
                message=(
                    f"Open: {open_ports}"
                    if open_ports
                    else "None of 25/465/587 reachable from this host — egress firewall, or the MX is filtered."
                ),
            )
        )
    except ValueError as e:
        ctx.add(WizardStep(name="SMTP ports", outcome="warn", message=f"Port scan rejected by scope: {e}"))
        open_ports = []
    ...
    ctx.add(
        WizardStep(
            name="SPF present", outcome="ok" if spf_ok else "warn", message="yes" if spf_ok else "missing"
        )
    )
    ctx.add(
        WizardStep(
    ...
    if first_ip:
        ...
        try:
            ...
            ctx.add(WizardStep(name="PTR for MX host", outcome="info", message=f"reverse lookup failed: {e}"))
