...
async def ssl_deep_inspect(ctx: WizardContext) -> list[Suggestion]:
    ...
    try:
        ...
        ctx.add(WizardStep(name="TLS handshake", outcome="fail",
    ...
    ctx.add(WizardStep(name="TLS handshake", outcome="ok",
                       message=f"Handshake succeeded against {host}:{port}"))

    # Hostname match
    sans = list(info.sans or [])
    hostname_ok = host in sans or any(
    ...
    ctx.add(WizardStep(
        ...
        message=("Hostname covered by SAN/CN" if hostname_ok else
                 f"Hostname {host} not in SAN list"),
