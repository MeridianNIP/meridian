...
async def why_isnt_my_domain_resolving(ctx: WizardContext) -> list[Suggestion]:
    ...
    # 1 · NS delegation
    ns = await run_dig(DigRequest(target=domain, record_type="NS", flags=("+short", "+noall", "+answer")))
    ...
    if prop.divergence:
        ctx.add(
            WizardStep(
                ...
                detail={
                    "unique_answers": list(prop.unique_answers),
                    "ok_resolvers": ok_count,
                    "total": len(prop.rows),
                },
            )
        )
    ...
    if any(s.name == "SOA serial match across authoritative NSes" and s.outcome == "warn" for s in ctx.steps):
        suggestions.append(
            Suggestion(
