...
async def mail_delivery(ctx: WizardContext) -> list[Suggestion]:
    ...
    if first_mx:
        ...
        aaaa_rows = _answers(aaaa.stdout)
        if not a_rows and not aaaa_rows:
            ctx.add(
                WizardStep(
                    name="Primary MX resolves", outcome="fail", message=f"{first_mx} has no A or AAAA records"
                )
            )
        else:
            ctx.add(
                WizardStep(
                    name="Primary MX resolves",
                    outcome="ok",
    ...
    ctx.add(
        WizardStep(
            ...
            message="DMARC policy published"
            if dmarc_rows
            else "No DMARC policy — receivers can't act on SPF/DKIM failures",
    ...
    ctx.add(
        WizardStep(
            ...
            message="MTA-STS deployed"
            if mta_sts_rows
            else "MTA-STS not published (optional; strong recommendation for modern mail)",
        )
    )
...
async def dnssec_chain(ctx: WizardContext) -> list[Suggestion]:
    ...
    for step in report.chain:
        ctx.add(
            WizardStep(
                ...
                if step.outcome in ("warn", "fail")
                else "info",
                message=step.message,
                detail={
                    "has_ds": step.has_ds,
                    "has_dnskey": step.has_dnskey,
    ...
    ctx.add(
        WizardStep(
            name="AD flag from recursor",
            outcome="ok" if report.ad_flag else "info",
            message=(
                "Recursor returned AD — chain validates"
                if report.ad_flag
                else "Recursor did not set AD — domain may be unsigned or chain broken"
            ),
        )
    )
    sug: list[Suggestion] = []
    if report.worst in ("fail", "warn"):
        sug.append(
            Suggestion(
                priority="critical" if report.worst == "fail" else "recommended",
...
async def axfr_wizard(ctx: WizardContext) -> list[Suggestion]:
    ...
    for row in report.rows:
        ctx.add(
            WizardStep(
                name=f"AXFR: {row.nameserver}", outcome="fail" if row.exposed else "ok", message=row.detail
            )
        )
    ...
    return [
        Suggestion(
            priority="info", title="All NSes refused AXFR", detail="Zone transfers are locked down. Good."
        )
    ]
...
async def registrar_mismatch(ctx: WizardContext) -> list[Suggestion]:
    ...
    if registrar_ns and live_ns and mismatch:
        ctx.add(
            WizardStep(
                name="Registrar vs live match",
                outcome="warn",
                message=f"{len(mismatch)} NS differ between registrar and live",
                detail={
                    "only_registrar": sorted(set(registrar_ns) - set(live_ns)),
        ...
        ctx.add(
            WizardStep(name="Registrar vs live match", outcome="ok", message="Registrar and live NS agree.")
        )
    return []
...
async def domain_bringup(ctx: WizardContext) -> list[Suggestion]:
    ...
    for label, rtype in checks:
        ...
        if rows:
            ...
            ctx.add(WizardStep(name=label, outcome="warn", message=f"No {rtype} returned"))
    ...
    if missing:
        return [
            Suggestion(
                ...
                detail="The following are absent or empty: "
                + ", ".join(missing)
                + ". Depending on the use case some may be optional (CAA if you don't care about cert-issuance restrictions).",
            )
        ]
    return [
        Suggestion(
            priority="info",
            title="All core records present",
...
async def cloudflare_validator(ctx: WizardContext) -> list[Suggestion]:
    ...
    ctx.add(
        WizardStep(
            ...
            message="Domain delegates to Cloudflare nameservers"
            if is_cf
            else "Not delegated to Cloudflare (that's fine if intended).",
            detail={"ns": ns_rows},
        )
    )
    ...
    dnskey = await run_dig(
        DigRequest(target=domain, record_type="DNSKEY", flags=("+short", "+noall", "+answer"))
    )
    ctx.add(
        WizardStep(
            ...
            message="Zone signed"
            if dnskey.stdout.strip()
            else "Zone unsigned — Cloudflare can enable DNSSEC in one click",
        )
    )
...
async def typosquat_sweep(ctx: WizardContext) -> list[Suggestion]:
    ...
    if len(raw) < 2:
        ctx.add(
            WizardStep(
                name="input", outcome="fail", message="Target must be a full domain like brand.example.com"
            )
        )
        return []
    ...
    ctx.add(
        WizardStep(
            ...
            message=(
                f"{len(registered)} variants resolve to something" if registered else "No variants are active"
            ),
...
async def dmarc_tuning(ctx: WizardContext) -> list[Suggestion]:
    ...
    ctx.add(WizardStep(name="DMARC record", outcome="ok", message=f"Parsed: {parts}", detail={"raw": body}))

    stage = {
        "none": "stage 1/3 (monitor)",
        "quarantine": "stage 2/3 (enforce soft)",
        "reject": "stage 3/3 (enforce hard)",
    }.get(policy, "unknown")
    ctx.add(WizardStep(name="Policy stage", outcome="info", message=f"p={policy} — {stage}"))
