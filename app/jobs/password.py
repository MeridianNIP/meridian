...
def _password_max_age_days(db: OrmSession) -> int:
    ...
    row = db.execute(text(
        "SELECT password_max_age_days FROM branding LIMIT 1"
    )).first()
...
def expiry_notify() -> dict[str, Any]:
    ...
    with session_scope() as db:
        ...
        for u in rows:
            ...
            try:
                dispatch(
                    db, event_kind="user.password_expiry_warning",
                    subject=subject, body=body,
                    payload={"user_id": str(u.id),
            ...
            """), {"uid": u.id, "t": threshold,
                   "cycle": u.password_changed_at, "now": now})
            sent += 1

        if sent:
            audit(db, action="password.expiry_notify",
                  payload={"sent": sent, "max_age_days": max_age})
