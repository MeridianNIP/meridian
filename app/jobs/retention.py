...
def audit_cleanup() -> dict[str, Any]:
    with session_scope() as db:
        ...
        audit(db, action="retention.audit_events.cleanup",
              payload={"rows_deleted": n, "keep_days": days})
...
def query_cleanup() -> dict[str, Any]:
    with session_scope() as db:
        ...
        """), {"cutoff": cutoff})
        n = result.rowcount or 0
        audit(db, action="retention.query_history.cleanup",
              payload={"rows_deleted": n, "keep_days": days})
        return {"rows_deleted": n, "keep_days": days}
...
def session_cleanup() -> dict[str, Any]:
    with session_scope() as db:
        ...
        """), {"cutoff": cutoff})
        n = result.rowcount or 0
        audit(db, action="retention.sessions.cleanup",
              payload={"sessions_revoked": n})
    ...
    with session_scope() as db:
        ...
        for table in ("celery_taskmeta", "celery_tasksetmeta"):
            ...
            if not exists:
                continue
            r = db.execute(text(
                f"DELETE FROM {table} WHERE date_done < :cutoff"
            ), {"cutoff": cutoff})
            purged[table] = r.rowcount or 0

        audit(db, action="retention.celery.purge",
              payload={"purged": purged, "keep_days": days})
...
def pcap_cleanup() -> dict[str, Any]:
    ...
    with session_scope() as db:
        ...
        audit(db, action="retention.pcap.cleanup",
              payload={"rows_deleted": removed_rows,
                       "files_removed": removed_files,
                       "keep_days": days})
        return {"rows_deleted": removed_rows,
                "files_removed": removed_files, "keep_days": days}
...
def file_quota() -> dict[str, Any]:
    ...
    """
    with session_scope() as db:
        hard_row = db.execute(text(
            "SELECT max_bytes FROM retention_rules WHERE scope='file_repo_user' AND enabled"
        )).scalar_one_or_none()
        hard_cap = int(hard_row) if hard_row is not None else 1024 * 1024 * 1024

        users_over = db.execute(text("""
        ...
        for row in users_over:
            ...
            evicted.append({"owner_id": str(row.owner_id),
                            "bytes_freed": freed,
                            "files_evicted": len(ids_evicted)})

        audit(db, action="retention.file_repo.quota",
              payload={"hard_cap_bytes": hard_cap,
                       "users_trimmed": len(evicted),
                       "evicted": evicted})
        return {"hard_cap_bytes": hard_cap, "users_trimmed": len(evicted),
                "evicted": evicted}
