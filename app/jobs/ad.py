...
def _to_filetime(dt: datetime) -> int:
    delta = dt.astimezone(timezone.utc) - _FILETIME_EPOCH
    ...
    with session_scope() as db:
        integrations = db.execute(
            ...
            .where(DirectoryIntegration.enabled.is_(True),
                   DirectoryIntegration.kind == "active_directory")
        ).scalars().all()
        ...
        for integ in integrations:
            try:
                ...
                results.append({"integration": integ.name, "error": str(e)[:200],
                                "stale_found": 0})
                continue
                ...
                with client._connect() as conn:
                    ...
                    for e in conn.entries:
                        ...
                        if isinstance(last_logon_raw, int):
                            ...
                            ll_dt = None
                        stale.append({
                            "sam": getattr(e, "sAMAccountName", {}).value if hasattr(e, "sAMAccountName") else None,
                            "upn": getattr(e, "userPrincipalName", {}).value if hasattr(e, "userPrincipalName") else None,
                            "dn": str(e.entry_dn),
            ...
            if has_table:
                ...
                """), {
                    ...
                    "i": integ.id, "d": days, "n": len(stale),
                    "s": __import__("json").dumps(stale[:50]),
                })

            results.append({"integration": integ.name,
                            "stale_found": len(stale),
                            "sample": stale[:5]})

        audit(db, action="ad.stale_accounts",
              payload={"days_threshold": days,
                       "integrations_scanned": len(integrations),
                       "results": results})
