...
_FAMILY: dict[str, str] = {
    ...
    "Apache-2.0": "permissive", "ISC": "permissive", "Unlicense": "public_domain",
    "Python-2.0": "permissive", "Zlib": "permissive", "X11": "permissive",
    ...
    "GPL-2.0-only": "strong_copyleft", "GPL-2.0-or-later": "strong_copyleft",
    "GPL-3.0-only": "strong_copyleft", "GPL-3.0-or-later": "strong_copyleft",
    "AGPL-3.0-only": "network_copyleft", "AGPL-3.0-or-later": "network_copyleft",
    "OFL-1.1": "font", "proprietary": "proprietary",
}
...
def _apt_packages() -> list[dict[str, str]]:
    ...
    for line in out.splitlines():
        ...
        rows.append({"name": name.strip(), "version": version.strip(),
                     "license_spdx": _apt_license(name.strip())})
    return rows
...
def _pip_packages() -> list[dict[str, str]]:
    ...
    for d in distributions():
        ...
        if not name:
            continue
        license_field = (md.get("License-Expression")
                         or md.get("License") or "").strip()
        ...
        if not license_field and md:
            for c in md.get_all("Classifier") or []:
                if c.startswith("License :: ") and "MIT" in c:
                    license_field = "MIT"
                    break
        rows.append({"name": name, "version": d.version or "",
...
def scan() -> dict[str, Any]:
    ...
    with session_scope() as db:
        ...
        for pkg in apt:
            _added, _license_changed = _upsert(
                db, name=pkg["name"], version=pkg["version"],
                category="os_package", license_spdx=pkg["license_spdx"],
                homepage=None, now=now,
            )
            added += int(_added)
            ...
            if _added and family in ("strong_copyleft", "network_copyleft"):
                strong_copyleft_new.append({
                    ...
                    "spdx": pkg["license_spdx"], "family": family,
                })

        for pkg in pip:
            _added, _license_changed = _upsert(
                db, name=pkg["name"], version=pkg["version"],
                category="python", license_spdx=pkg["license_spdx"],
                homepage=pkg.get("homepage"), now=now,
            )
            added += int(_added)
            license_changes += int(_license_changed)
            family = _family(pkg["license_spdx"])
        ...
        db.execute(text("""
            UPDATE oss_scan_runs SET completed_at = :t, status = 'ok',
             ...
             WHERE id = :id
        """), {
            "t": datetime.now(timezone.utc), "a": added, "r": removed,
            "lc": license_changes,
            "d": json.dumps({"apt_count": len(apt), "pip_count": len(pip),
        ...
        """), {"c": json.dumps(sbom),
               "n": len(sbom.get("components") or [])})

        audit(db, action="oss.scan",
              payload={"apt_count": len(apt), "pip_count": len(pip),
        ...
        if strong_copyleft_new:
            try:
                ...
                dispatch(
                    db, event_kind="oss.copyleft_new",
                    subject=f"{len(strong_copyleft_new)} new strong-copyleft component(s)",
                    body="\n".join(f"{c['name']} {c['version']} · {c['spdx']} ({c['family']})"
    ...
    return {"apt": len(apt), "pip": len(pip), "added": added,
            "removed": removed, "license_changes": license_changes,
            "strong_copyleft_new": strong_copyleft_new}


def _upsert(db, *, name: str, version: str, category: str,
            license_spdx: str, homepage: str | None, now: datetime) -> tuple[bool, bool]:
    family = _family(license_spdx)
    existing = db.execute(text("""
        SELECT id, license_spdx FROM oss_components
         WHERE name = :n AND version = :v AND category = :c
    """), {"n": name, "v": version, "c": category}).first()
        ...
        db.execute(text("""
            UPDATE oss_components
               SET last_seen = :t, license_spdx = :s, license_family = :f,
                   ...
                   removed_at = NULL
             WHERE id = :id
        """), {"t": now, "s": license_spdx or "unknown", "f": family,
               "h": homepage, "id": existing.id})
    ...
    """), {"n": name, "v": version, "c": category,
           "s": license_spdx or "unknown", "f": family,
           "h": homepage, "t": now})
    return (True, False)
...
def _build_sbom_cyclonedx(db) -> dict:
    ...
    for r in rows:
        components.append({
            ...
            "externalReferences": ([{"type": "website", "url": r.homepage_url}]
                                   if r.homepage_url else []),
        })
    return {
