...
async def asn_lookup(ip: str, *, timeout_s: float = 8.0) -> dict[str, Any]:
    ...
    return {"ip": ip, "asn": None, "raw": r.stdout[:4096],
...
async def bgp_looking_glass(target: str, *, timeout_s: float = 12.0) -> dict[str, Any]:
    ...
    """
    headers = {"User-Agent": "Meridian-NIP/1.0 (bgp-lg)",
    ...
    async with httpx.AsyncClient(timeout=timeout_s, headers=headers) as c:
        if target.upper().startswith("AS"):
            ...
            r = await c.get("https://stat.ripe.net/data/as-overview/data.json",
                            params={"resource": f"AS{n}"})
            r.raise_for_status()
        ...
        net_r = await c.get("https://stat.ripe.net/data/network-info/data.json",
        ...
        async def _holder(n: int) -> dict[str, Any]:
            rr = await c.get("https://stat.ripe.net/data/as-overview/data.json",
                             params={"resource": f"AS{n}"})
...
_DNSBLS: tuple[tuple[str, str, str, str], ...] = (
    ("zen.spamhaus.org",   "Spamhaus ZEN",   "Combined SBL/XBL/PBL (spam + exploit + policy)",
     "https://check.spamhaus.org/results/?query={ip}"),
    ("b.barracudacentral.org", "Barracuda",  "Barracuda Reputation Block List",
     "https://www.barracudacentral.org/lookups/lookup-reputation"),
    ("bl.spamcop.net",     "SpamCop",        "Spam-reporting cooperative",
     "https://www.spamcop.net/w3m?action=checkblock&ip={ip}"),
    ("dnsbl.sorbs.net",    "SORBS",          "Composite abuse / open-relay list",
     "http://www.sorbs.net/lookup.shtml?{ip}"),
    ("cbl.abuseat.org",    "CBL",            "Composite Blocking List (merged into Spamhaus XBL)",
     "https://www.abuseat.org/lookup.cgi?ip={ip}"),
    ("psbl.surriel.com",   "PSBL",           "Passive Spam Block List",
     "https://psbl.org/listing?ip={ip}"),
    ("dnsbl.dronebl.org",  "DroneBL",        "Active-threat drone/bot list",
     "https://dronebl.org/lookup?ip={ip}"),
    ("ubl.unsubscore.com", "Lashback UBL",   "Lashback Unsubscribe Block List",
...
async def ip_reputation(ip: str, *, timeout_s: float = 3.0) -> dict[str, Any]:
    ...
    async def _one(zone: str, label: str, desc: str, url_tmpl: str) -> dict[str, Any]:
        ...
        r = await sandbox_run(
            "dig", [query, "A", "+short", "+time=2", "+tries=1"],
            timeout_s=timeout_s,
        )
        listed = any(l.strip().startswith("127.") for l in r.stdout.splitlines())
        return {"list": label, "zone": zone, "description": desc,
    ...
    return {"ip": ip, "lists_checked": len(rows), "listed_on": len(hits),
            "rows": rows, "hits": hits}
...
async def ip_geolocate(ip: str, *, timeout_s: float = 8.0) -> dict[str, Any]:
    ...
    async with httpx.AsyncClient(timeout=timeout_s,
                                 headers={"User-Agent": "Meridian-NIP/1.0 (geoip)"}) as c:
        r = await c.get(f"https://ipapi.co/{ip}/json/")
        if r.status_code in (429, 403):
            return {"ip": ip, "error": "upstream rate-limited (ipapi.co)",
                    "rate_limited": True}
        r.raise_for_status()
        d = r.json()
    if d.get("error"):
...
def _check_hsts(headers: dict[str, str]) -> tuple[str, str, str]:
    ...
    if not v:
        return ("fail", "missing — site is replayable over HTTP",
                "Add:  Strict-Transport-Security: max-age=31536000; includeSubDomains; preload")
    max_age = 0
    for part in v.split(";"):
        part = part.strip()
        if part.startswith("max-age="):
            try: max_age = int(part[len("max-age="):])
            except ValueError: pass
    if max_age < 15768000:   # 6 months
        return ("warn", f"present but max-age={max_age}s (<6 months)",
...
def _check_csp(headers: dict[str, str]) -> tuple[str, str, str]:
    ...
    if not v:
        return ("fail", "missing — scripts can load from anywhere",
    ...
    if "unsafe-inline" in v.lower():
        return ("warn", "allows 'unsafe-inline' — negates most XSS protection",
...
def _check_frame_options(headers: dict[str, str]) -> tuple[str, str, str]:
    ...
    if not v:
        return ("fail", "missing — site can be iframed for clickjacking",
...
def _check_xcto(headers: dict[str, str]) -> tuple[str, str, str]:
    ...
    if not v or "nosniff" not in v.lower():
        return ("fail", "missing 'nosniff' — browsers may MIME-sniff and misinterpret",
...
def _check_referrer(headers: dict[str, str]) -> tuple[str, str, str]:
    ...
    if not v:
        return ("warn", "missing — default referrer leaks full URLs cross-origin",
...
def _check_permissions(headers: dict[str, str]) -> tuple[str, str, str]:
    ...
    if not v:
        return ("warn", "missing — browser features (geolocation, camera, …) aren't gated",
...
def _check_cookie_flags(headers: dict[str, str]) -> tuple[str, str, str]:
    ...
    issues = []
    fixes = []
    if "httponly" not in v.lower(): issues.append("missing HttpOnly"); fixes.append("HttpOnly (blocks JS access)")
    if "secure" not in v.lower():   issues.append("missing Secure");   fixes.append("Secure (HTTPS-only)")
    if "samesite" not in v.lower(): issues.append("missing SameSite"); fixes.append("SameSite=Lax (or Strict)")
    if issues:
        return ("warn", "; ".join(issues),
                "Set-Cookie flags to add: " + ", ".join(fixes))
...
async def cve_lookup(cve_id: str, *, timeout_s: float = 15.0) -> dict[str, Any]:
    ...
    async with httpx.AsyncClient(timeout=timeout_s,
                                 headers={"User-Agent": "Meridian-NIP/1.0 (cve-lookup)",
    ...
    if not items:
        return {"cve": cve_id, "error": "no vulnerabilities returned",
                "refs": _cve_external_refs(cve_id)}
    v = (items[0] or {}).get("cve") or {}
    # Description: EN preferred.
    desc = next((d.get("value") for d in (v.get("descriptions") or [])
                 if d.get("lang") == "en"), "")
    ...
    for weakness in (v.get("weaknesses") or []):
        for d in (weakness.get("description") or []):
            ...
            if val and val not in cwes:
                cwes.append(val)
    refs = [{"url": r.get("url"), "tags": r.get("tags") or []}
            for r in (v.get("references") or [])][:50]
    return {
        "cve": cve_id,
...
async def security_header_audit(url: str, *, timeout_s: float = 10.0) -> dict[str, Any]:
    ...
    async with httpx.AsyncClient(timeout=timeout_s, follow_redirects=True,
                                 headers={"User-Agent": "Meridian-NIP/1.0 (header-audit)"}) as c:
        r = await c.get(url)
...
async def _load_kev(*, timeout_s: float = 20.0) -> dict[str, Any]:
    ...
    async with httpx.AsyncClient(timeout=timeout_s,
                                 headers={"User-Agent": "Meridian-NIP/1.0 (kev)",
                                          "Accept": "application/json"}) as c:
...
async def epss_lookup(cve_id: str, *, timeout_s: float = 8.0) -> dict[str, Any]:
    ...
    if not _CVE_RE.match(cve_id):
        raise ValueError(f"not a valid CVE id: {cve_id!r}")
    async with httpx.AsyncClient(timeout=timeout_s,
                                 headers={"User-Agent": "Meridian-NIP/1.0 (epss)",
                                          "Accept": "application/json"}) as c:
...
async def circl_lookup(cve_id: str, *, timeout_s: float = 10.0) -> dict[str, Any]:
    ...
    if not _CVE_RE.match(cve_id):
        raise ValueError(f"not a valid CVE id: {cve_id!r}")
    async with httpx.AsyncClient(timeout=timeout_s,
                                 headers={"User-Agent": "Meridian-NIP/1.0 (circl)",
                                          "Accept": "application/json"}) as c:
    ...
    summary = next((d.get("value") for d in (cna.get("descriptions") or [])
    ...
    for m in (cna.get("metrics") or []):
        for key, ver in (("cvssV3_1", "3.1"), ("cvssV3_0", "3.0"),
                         ("cvssV4_0", "4.0"), ("cvssV2_0", "2.0")):
            block = m.get(key)
            if block:
    ...
    for a in (cna.get("affected") or []):
        ...
        affected.append(" · ".join(p for p in (
            f"{vendor}/{product}".strip("/"),
            ", ".join(vers[:4]) + ("…" if len(vers) > 4 else ""),
        ) if p))

    refs = [{"url": r.get("url"), "tags": r.get("tags") or []}
            for r in (cna.get("references") or []) if r.get("url")][:50]
...
async def dshield_lookup(ip: str, *, timeout_s: float = 8.0) -> dict[str, Any]:
    ...
    async with httpx.AsyncClient(timeout=timeout_s,
                                 headers={"User-Agent": "Meridian-NIP/1.0 (dshield)",
                                          "Accept": "application/json"}) as c:
...
def _resolve_threat_intel(kind: str) -> tuple[str, dict[str, Any]]:
    ...
    with session_scope() as db:
        ...
        if not row:
            ...
            src_row = db.execute(_sql_text(
                "SELECT enabled FROM threat_intel_sources WHERE source_key=:k"
            ), {"k": kind}).first()
...
# ============================================================================
async def abuseipdb_lookup(ip: str, *, timeout_s: float = 10.0,
    ...
    async with httpx.AsyncClient(timeout=timeout_s,
                                 headers={"Key": key, "Accept": "application/json",
                                          "User-Agent": "Meridian-NIP/1.0"}) as c:
        r = await c.get(f"{base}/check",
...
# ============================================================================
async def urlscan_search(query: str, *, timeout_s: float = 15.0,
    ...
    async with httpx.AsyncClient(timeout=timeout_s,
        ...
        r = await c.get(f"{base}/search/",
                        params={"q": q, "size": max(1, min(size, 100))})
        r.raise_for_status()
        j = r.json() or {}
    rows = []
...
async def shodan_lookup(ip: str, *, timeout_s: float = 20.0) -> dict[str, Any]:
    ...
    async with httpx.AsyncClient(timeout=timeout_s,
                                 headers={"User-Agent": "Meridian-NIP/1.0"}) as c:
    ...
    return {
        ...
        "vulns":       list((j.get("vulns") or {}))[:50] if isinstance(j.get("vulns"), dict) else (j.get("vulns") or [])[:50],
        "tags":        j.get("tags") or [],
    ...
    for s in (result.get("services") or [])[:50]:
        svcs.append({
            ...
            "banner_sha256":   (s.get("banner_hashes_sha256") or [None])[0]
                                 if isinstance(s.get("banner_hashes_sha256"), list) else None,
            "observed_at":     s.get("observed_at"),
            "product":         (s.get("software") or [{}])[0].get("product")
                                 if s.get("software") else None,
            "version":         (s.get("software") or [{}])[0].get("version")
                                 if s.get("software") else None,
        })
