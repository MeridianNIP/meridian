...
def _due_monitors(db: OrmSession, now: datetime) -> list[Monitor]:
    ...
    from app.safety.limits import MONITOR_INTERVAL_CEILING_S, MONITOR_INTERVAL_FLOOR_S, clamp
    ...
    return [
        ...
        or (now - m.last_sample_at)
        >= timedelta(
            seconds=(
                int(
                    clamp(
                        m.interval_seconds, floor=MONITOR_INTERVAL_FLOOR_S, ceiling=MONITOR_INTERVAL_CEILING_S
                    )
                )
                - 1
            )
...
async def _sample_one(db: OrmSession, m: Monitor, *, now: datetime) -> dict[str, Any]:
    ...
    return {"id": str(m.id), "status": result.status, "value": result.value, "error": result.error}
